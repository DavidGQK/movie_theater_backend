...
if __name__ == '__main__':
    es_url = os.getenv('BASE_ES_URL', default='http://localhost:9200')
    index_name = os.getenv('ES_INDEX_NAME', default='movies')
    print(es_url)
    with conn_context('db.sqlite') as connection:
        loader = ESLoader(url=es_url)
        etl = ETL(conn=connection, es_loader=loader)
        etl.load(index_name=index_name)