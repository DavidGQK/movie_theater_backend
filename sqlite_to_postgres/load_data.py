    ...
    def _load_table(self, table_name: str) -> List[dataclass]:
        ...
        while True:
            ...
            if next_row:
                for field_name, field_type in current_value_type:
                    if next_row[field_name] is None:
                        ...
                        next_row[field_name] = sanitize_field(
                            field_type=field_type, field_value=next_row[field_name]
                        )
                ...
                logger.info(
                    "Data loaded from table: {}, {} rows".format(
                        table_name, len(current_table)
                    )
                )
...
class PostgresSaver:
    def __init__(self, pg_conn: _connection, page_size: int, schema: str = "content"):
        self.cursor = pg_conn.cursor()
        self.schema = schema
        self.page_size = page_size

    def _save_data_to_table(self, table_name: str, table_data: List[dataclass]) -> None:
        ...
        logger.info(f"Uploaded {table_size} rows for table:{table_name}")

    def save_all_data(self, data: dict, tables: Tuple[str]) -> None:
        for table_name in tables:
            self._save_data_to_table(table_name=table_name, table_data=data[table_name])
...
def main():
    ...
    try:
        with psycopg2.connect(**dsl, cursor_factory=DictCursor) as pg_conn:
            ...
            postgres_saver.save_all_data(data=movies_data, tables=tables_names)
